...
class AISnake:
    ...
    def check_collision(self, obstacle_coords):
        ...
        if x < 0 or x >= WIDTH or y < 0 or y >= HEIGHT:
            ...
            self.game.on_ai_snake_died("Game Over! AI Snake hit the boundary.")

        # Snake colliding with a obstacle
        for coords in obstacle_coords:
            if is_collided(coords):
                self.game.on_ai_snake_died("Game Over! AI Snake hit an obstacle.")

        # Snake colliding with itself
        for body_part in self.coordinates[1:]:
            if is_collided(body_part):
                self.game.on_ai_snake_died("Game Over! AI Snake collided with itself.")

        for body_part in self.game.snake:
            if is_collided(body_part):
                self.game.on_ai_snake_died("Game Over! AI Snake collided with User Snake.")
...
class SnakeGame:
    def __init__(self, master):
        ...
        self.canvas = tk.Canvas(self.master, bg="black", width=WIDTH, height=HEIGHT)
        self.canvas.pack()
    ...
    def on_snake_died(self, msg: str):
        ...
        self.canvas.create_text(
            ...
            80,
            font=('consolas', 12),
            text="User Snake Died",
            fill="red",
            tags="user_snake_died",
        )
    ...
    def setup_score_board(self):
        ...
        self.canvas.create_text(
            ...
            30,
            font=('consolas', 12),
            text="User Points: {}".format(self.score),
            fill="red",
            tags="user_points",
        )
        self.canvas.create_text(
    ...
    def game_over(self, msg):
        ...
        if self.score < self.ai_score:
            won_message = "AI Snake Wins!"
        elif self.score > self.ai_score:
            won_message = "User Snake Wins!"
        else:
            won_message = "It's a tie!"
        self.canvas.create_text(
        ...
        self.canvas.create_text(
            ...
            font=('consolas', 20),
            text="User Points: {},  AI Points: {}".format(self.score, self.ai_score),
            fill="red",
            tags="gameover",
        )
    ...
    def check_collision(self):
        ...
        for body_part in self.snake[1:]:
            if x == body_part[0] and y == body_part[1]:
                self.on_snake_died("Game Over! Snake collided with itself.")

        for body_part in self.ai_snake.coordinates:
            if x == body_part[0] and y == body_part[1]:
                self.on_snake_died("Game Over! Snake collided with AI Snake.")
